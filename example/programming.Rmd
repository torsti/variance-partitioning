---
title: Programming supplement
subtitle: Variance partitioning tutorial
author:
  - Torsti Schulz
  - Marjo Saastamoinen
  - Jarno Vanhatalo
...
Specifically, the accompanying RMarkdown source file can be consulted to see the code for generating the tables and figures.
...
Though less versatile than the Bayesian modelling package `brms`, its syntax is very similar to traditional R regression packages and libraries.
...
We use for the demonstration the same Glanville fritillary dataset as in the main study.
The corresponding file in the datadryad dataset is `data/survey_data.tsv` inside the archive `ECOG-04799.zip` (Schulz *et al.* 2019, 2020).

```{r label = 'load data'}
dat <- read.delim('survey_data.tsv', na.strings = 'NULL')
```
...
We model the (log) growth rate using the linearized Ricker model (see, e.g., Weigell et al., 2021) and fit it using `rstanarm`:
...
To form the matrix of linear terms, $A$ (See the Mathematical supplement), when working with a (generalized) linear model we can use the design matrix, $X$, of the model.
...
For calculating the different variance partitioning measures for groups of linear terms we need to define the grouping (see below and the Mathematical supplement).
...
To simplify the code and improve readability, we can define auxliary constants for the number of linear terms, number groups of linear terms, and number of posterior samples (or *iterations*).
...
The sample covariance matrix of the linear terms, $K_A$, can be used to calculate all the variance partitioning measures presented in this paper (and many others; see the Mathematical supplement).
...
This is the case also when we calculate the diagonal variance partitioning $P_x$ as illustrated below.
...
The elements of the matrix are one if the linear term of the corresponding row is in the group of the respective column and zero otherwise (see Mathematical supplement).
...
### Diagonal variance partition $P_b$

In matrix notation the diagonal variance partition is calculated as $V_l = \frac{K_{B_{l,l}}}{\mathrm{tr}(K_B)}$ where $\mathrm{tr}()$ is the trace of the matrix, that is the sum of its diagonal terms.
...
```{r echo = FALSE}
print_vp_table('P_b', groups_pretty, caption = 'Summaries of the posterior distribution of the diagonal variance partition over groups of linear terms.')
```

```{r echo = FALSE}
vp_plot(P_b, 'palevioletred', groups_pretty)
```
...
### Unique variance partition $U_b$

The unique variance partition is calculated as the sum of the residual variance of the least squares prediction of a group of linear terms as a function of the linear terms in all the other groups.
In practice this is simple to implement using the sample covariance matrix $K_A$ and the grouping matrix $B$.

$$U_l = \frac{1_{|B_l|}'(K_{A_{B_l,B_l}} - K_{A_{B_l,B_{\setminus l}}} K_{A_{B_{\setminus l},B_{\setminus l}}}^{-1} K_{A_{B_{\setminus l}, B_l}})1_{|B_l|}}{\mathrm{Var}[y]}$$
...
```{r echo = FALSE}
print_vp_table('U_b', groups_pretty, caption = 'Summaries of the posterior distribution of the unique variance partition over groups of linear terms.')
```

```{r echo = FALSE}
vp_plot(U_b, 'darkgoldenrod2', groups_pretty)
```
...
For example, if the prediction data included years not part of the sample data, we would have to sample values for the new years using the *iid* random effect's distribution; see the Mathematical supplement and the Code supplement for the case study results discussed in the article for an example on spatial random effects.
...
The matrix $H$ can be used to generate a matrix of between-condition linear term means, which we use to calculate the covariance matrix for both within and between conditions (see also Mathematical supplement).
...
Schulz, T., J. Vanhatalo, and M. Saastamoinen. 2020. Long-term demographic surveys reveal a consistent relationship between average occupancy and abundance within local populations of a butterfly metapopulation. *Ecography* 43: 306--317.

Weigel, B., J. Mäkinen, M. Kallasvuo, and J. Vanhatalo. 2021. Exposing changing phenology of fish larvae by modeling climate effects on temporal early life-stage shifts. *Marine Ecology Progress Series* 666:135--148
